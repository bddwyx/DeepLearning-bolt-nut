...
def parse_args():
    ...
    add_arg('freeze_model_save_dir',   str,    'freeze_model',     "The path to save model.")
    add_arg('pretrained_model', str,    'imagenet_resnet50_fusebn', "The init model path.")
    
    add_arg('class_num',        int,   3,          "Class number.")
    add_arg('data_dir',         str,   'data/data6045/train.txt',        "The data root path.")
    
    #SOLVER
    add_arg('learning_rate',    float,  0.001,     "Learning rate.")
    add_arg('max_iter',         int,    180000,   "Iter number.")
    add_arg('epoch',         int,    100,   "epoch number.")
    ...
    add_arg('batch_size_per_im',int,    256,    "fast rcnn head batch size")
