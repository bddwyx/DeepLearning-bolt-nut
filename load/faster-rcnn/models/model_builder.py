...
class RCNN(object):
    ...
    def build_model(self, image_shape):#构建网络正向流程
        ...
        # Mask RCNN
        if cfg.MASK_ON:
            self.mask_rcnn_heads(self.body_conv)
    ...
    def build_input(self, image_shape):#############所有需要输入的量
    ...
    def feeds(self):#######返回在训练过程中需要传入数据的量
        if self.mode == 'infer':
            return [self.image, self.im_info]
        if self.mode == 'val':
    ...
    def rpn_heads(self, rpn_input):
        ...
        self.rpn_rois, self.rpn_roi_probs = fluid.layers.generate_proposals(
            ...
            eta=eta)
        if self.mode == 'train':
            outs = fluid.layers.generate_proposal_labels(           #构建rois的的label,构造方式同302行介绍
    ...
    def rpn_loss(self):########计算rpn_loss,包括分类损失和box回归损失
        ...
        ###所有的anchor与gt_box计算IOU，大于阈值的为正类，小于阈值的为负类。，并非全部anchor都用于计算loss，挑选若干score_top正样本anchor和负样本anchor用于计算loss
        
        score_pred, loc_pred, score_tgt, loc_tgt, bbox_weight = \
            fluid.layers.rpn_target_assign(
                bbox_pred=rpn_bbox_pred_reshape,
                cls_logits=rpn_cls_score_reshape,
                anchor_box=anchor_reshape,#所有的anchor
                anchor_var=var_reshape,
                gt_boxes=self.gt_box,  #单张图片的box,数据格式为[n,4]，n为gt_box的个数,不需要经过归一化
                is_crowd=self.is_crowd,  #是否为密集，一般为0
                im_info=self.im_info,   #是否为密集，一般为0
